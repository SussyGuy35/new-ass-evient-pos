...
chromium-browser --user-data-dir="$CHROME_PROFILE" --kiosk --disable-dev-shm-usage --disable-extensions --disable-background-networking --disable-sync --disable-translate --no-first-run --disable-infobars --autoplay-policy=no-user-gesture-required http://localhost:8000/login.html
